...
class RAGService:
    ...
    async def query(
        ...
        dense_weight = cfg.rag_dense_weight or self._settings.rag_dense_weight
        sparse_weight = cfg.rag_sparse_weight or self._settings.rag_sparse_weight

        effective_question = question
        if rewrite_enabled:
            effective_question = await self._rewrite_question(question)

        query_embedding = await self._tei.embed([effective_question], cfg.embedding_dimension)
